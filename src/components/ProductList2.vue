  ...
  <div class="pro_list">
     ...
     <swiper :options="swiperOption" ref="mySwiper" >
      ...
      <!--  <ul ref="list" v-if="list.length>0"> -->
           ...
           <router-link tag="div" v-bind:to="{name:'pro_details',params:{'id':v.id,'cid':cid}}" v-for="v in list" :key="v.id" class="swiper-slide">
...
export default {
  ...
  created:function(){
      //组件被创建时，获取分类id,根据分类id，取分类下的数据（ajax）
      var _this = this; //当前组件
      axios.get("./static/data/product.json")
      .then(function(response){
          // response 成功时调用，返回的结果(数据及ajax请求信息，数据是在data里面)
          // console.log(response.data)
          // console.log(JSON.parse(response.data))
          //取分类对应的产品列表
          var pro_list = response.data;
         
          if(pro_list.length>0){
            for(var i=0;i<pro_list.length;i++){
              // console.log(pro_list[i]);//
              if(_this.cid == pro_list[i].class_id){
                _this.list = pro_list[i].data;//把当前产品列表放到list变量中
                break;
              }
            }
         }
