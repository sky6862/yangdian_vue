...
$p = mysql_select_db("vue-app",$con);//数据名称：book_d0801
...
switch($do){
	...
	case "list":
		...
		if(!empty($rs)){
			echo json_encode(array('status'=>1,'msg'=>'ok！',"data"=>$rs,"username"=>$_COOKIE["username"],"uid"=>$_COOKIE['uid'],"page_count"=>$page_total));
		}else{
	...
	case "support":
		...
		break;
    default:
        echo json_encode(array('status'=>0,'msg'=>'非法请求，缺少参数!'));
		break;
}